...
if args.saved_expr == "":
    ...
    OUTPATH = os.path.join(basedir, 'Results', 'RNATracker-10foldcv',
...
'''load model'''
model = RNATracker(4000, nb_classes, OUTPATH, 10)
model.build_model_advanced_masking(nb_filters=32, filters_length=10, pooling_size=3, lstm_units=32,
                                   embedding_vec=seq_encoding_vectors)
model.model.load_weights(
    os.path.join(basedir, 'Results/RNATracker-10foldcv/all_in_one-2018-08-20-00-35-45/weights_fold_10.h5'))
...
for i, (id, true_length, x, y) in enumerate(zip(reversed(ids), reversed(true_length), reversed(X), reversed(y))):
    ...
    original_predict = model.model.predict(x.reshape(1, -1)).reshape(-1, )
    ...
    if np.max(y) > 0.5 and np.argmax(original_predict) == np.argmax(y) and utr3_length != 0:
        ...
        if os.path.exists(os.path.join(MAT_OUTPATH, '{0}_{1}_{2}.npy'.format(id, true_length, utr3_length))):
            ...
            for i in range(4000 - true_starting_index):
                ...
                if args.permute_mask:
                    ...
                    preds = model.model.predict(np.concatenate(batch, axis=0))
            ...
            if args.permute_mask:
                ...
                preds = model.model.predict(np.concatenate(tmp, axis=0))
        ...
        for thresholding in kl_threshold:
            ...
            for i in non_overlapping_indices:
                ...
                x_[true_starting_index + i - effective_left:
                   true_starting_index + i + effective_right] = 0
                masked_predict = model.model.predict(x_.reshape(1, -1)).reshape(-1, )
